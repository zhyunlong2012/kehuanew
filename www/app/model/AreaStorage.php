 ...
 * 库位库存表格
 */
class AreaStorage extends Model{
    // 设置字段信息
    protected $schema = [
        'id'                 => 'int',
     ...
     * @return void
     */
    public function add($position_area_id,$product_id,$amount,$factory_id=''){
        $map = [
            ['position_area_id','=' , $position_area_id],
            ['product_id','=',$product_id],
            ['factory_id','=',$factory_id]
        ];
        $res = AreaStorage::where($map)->find();
        if ($res) {
            // if($amount<0){
            ...
            if($res->save() == true){
                // addLog('成功!更改库位库存,产品id:'.$product_id.',库位id'.$position_area_id.',数量'.$amount);
                return true;
            }else{
                // addLog('失败!更改库位库存,产品id:'.$product_id.',库位id'.$position_area_id.',数量'.$amount);
                return false;
            }
        }else{
            //未有库存，新增库存
     ...
     * @return void
     */
    public function nowSto($current,$pageSize,$position_id,$position_area_id,$pro_code,$factory_id,$excel=false,$other_code=''){
        // $map1 = [];
        $map = [];
