 ...
 * 供应商分类表格
 */
class FactoryCates extends Model{
    use SoftDelete;
    ...
    public function add($pid,$cates_name,$cates_code,$status=1){
        $res = FactoryCates::where('cates_code', $cates_code)->findOrEmpty();
        if (!$res->isEmpty()) {
            return $this->updata($res->id,$pid,$cates_name,$cates_code,$status);
        }

        $factory_cates_model = new FactoryCates();
        $factory_cates_model->pid = $pid;
        $factory_cates_model->cates_name = $cates_name;
        $factory_cates_model->cates_code = $cates_code;
        $factory_cates_model->status = $status;
        addLog('添加供应商分类'.$cates_name);
        if($factory_cates_model->save() == true){
            return true;
        }else{
            return false;
        }
    ...
    public function updata($id,$pid,$cates_name,$cates_code,$status=1){
        ...
        $res = FactoryCates::find($id);
        if (empty($res)) {
            return false;
        }
        addLog('修改供应商分类:'.$cates_name);
        $res->pid = $pid;
    ...
    public function del($ids = []){
        ...
        return FactoryCates::destroy($ids);
    }
